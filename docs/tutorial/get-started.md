...
* [Install `cert-manager`](https://cert-manager.io/docs/installation/). You can quickly install `cert-manager` by
  ...
  ```
* [Install `kernel-devel` or `linux-headers` on the host](#TODO)
* [Install `kubectl` version `>= 1.22`](https://kubernetes.io/docs/tasks/tools/)

## 1: Install Piraeus Operator

In this tutorial we will be using `kubectl` the built-in `kustomize` feature to deploy Piraeus Operator.
...
We still have not configured any storage location for our volumes. This can be accomplished by creating a new
...
This will cause some Pods to be recreated, afterwards the `linstor node list`, will temporarily show offline nodes
...
the storage pools where configured:
...
volumes. In the `StorageClass`, we specify the storage pool from above, and that we want our data to be replicated on
three nodes:
...
metadata:
  name: data-volume
spec:
  storageClassName: piraeus-three-replicas
  resources:
    requests:
      storage: 1G
  accessModes:
...
When we check the created PersistentVolumeClaim, we can see that it remains in `Pending` state. We first need to create
a "consumer", in this case is just a `Pod`.

```
$ kubectl get persistentvolumeclaim
NAME          STATUS    VOLUME   CAPACITY   ACCESS MODES   STORAGECLASS             AGE
data-volume   Pending                                      piraeus-three-replicas   14s
```

For our consumer, we will create a Deployment for a simple web server, serving files from our volume.
...
$ kubectl get persistentvolumeclaim
NAME          STATUS   VOLUME                                     CAPACITY   ACCESS MODES   STORAGECLASS             AGE
data-volume   Bound    pvc-9e1149e7-33db-47a7-8fc6-172514422143   976563Ki   RWO            piraeus-three-replicas   1m
```
...
Taking a look with the `linstor` client, we can see that the volume data is replicated on three nodes.
...
| n2.example.com | pvc-9e1149e7-33db-47a7-8fc6-172514422143 | pool1       |     0 |    1000 | /dev/drbd1000 |   832 KiB | Unused | UpToDate |
| n3.example.com | pvc-9e1149e7-33db-47a7-8fc6-172514422143 | pool1       |     0 |    1000 | /dev/drbd1000 |   832 KiB | Unused | UpToDate |
+-------------------------------------------------------------------------------------------------------------------------------------------+
```

We have now successfully set up Piraeus Datastore and used it to provision a replicated Persistent Volume in a
Kubernetes cluster.
